...
describe("ForefrontAntiSpamReport add method", () => {
    let forefrontAntiSpamReport: ForefrontAntiSpamReport;

    beforeEach(() => {
        forefrontAntiSpamReport = new ForefrontAntiSpamReport();
    });

    test("should return true and parse the header when header name matches", () => {
        const value = "CIP:208.75.123.162;CTRY:US;";
        const header = new Header("X-Forefront-Antispam-Report", value);
        const result = forefrontAntiSpamReport.add(header);
        expect(result).toBe(true);
        expect(forefrontAntiSpamReport.rows.some(row => row.value === "US")).toBe(true);
    });

    test("should return false when header name does not match", () => {
        const value = "CIP:208.75.123.162;CTRY:US;";
        const header = new Header("X-Other-Header", value);
        const result = forefrontAntiSpamReport.add(header);
        expect(result).toBe(false);
        expect(forefrontAntiSpamReport.rows.some(row => row.value === "US")).toBe(false);
    });

    test("should return formatted string when rows have values", () => {
        const value = "CIP:208.75.123.162;CTRY:US;";
        const header = new Header("X-Forefront-Antispam-Report", value);
        forefrontAntiSpamReport.add(header);
        const result = forefrontAntiSpamReport.toString();
        expect(result).toBe(
    ...
    test("should return empty string when no rows have values", () => {
        const result = forefrontAntiSpamReport.toString();
        expect(result).toBe("");
    });
});
